...
const handleWebhook = async (
    ...
    switch (event.type) {
        case 'checkout.session.completed': {
            ...
            // Map to onCheckoutUpdated with 'succeeded' status for compatibility
            await handlers.onCheckoutUpdated?.({ ...data, status: 'succeeded' })

            // If this is a subscription checkout, also fire onSubscriptionActive
